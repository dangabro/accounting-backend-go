package controller

import (
	"database/sql"
	"github.com/dgb9/db-account-server/internal/config"
	"github.com/dgb9/db-account-server/internal/data"
	"github.com/dgb9/db-account-server/internal/service"
	"net/http"
)

type accountType struct {
	config config.Config
	db     *sql.DB
}

func AccountType(config config.Config, db *sql.DB) http.Handler {
	return &accountType{
		config: config,
		db:     db,
	}
}

func (h *accountType) ServeHTTP(writer http.ResponseWriter, r *http.Request) {
	success, err := h.process(r)

	if err != nil {
		_ = writeJsonResponse(writer, err)
	} else {
		_ = writeJsonResponse(writer, success)
	}
}

func (h *accountType) process(r *http.Request) ([]data.AccountTypeData, error) {
	var res []data.AccountTypeData

	token, err := getToken(r)
	if err != nil {
		return res, err
	}

	serv := service.New(h.db, h.config)
	_, err = serv.ValidateToken(token)
	if err != nil {
		return res, err
	}

	res, err = serv.GetAccountTypeCds()

	return res, err
}
